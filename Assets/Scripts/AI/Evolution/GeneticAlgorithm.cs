...
public class GeneticAlgorithm
{
    ...
    public string PathToInitGenotypeFrom { get; private set; }
    public float bestReward;
    public static float averageEvaluation;
    ...
    /// </remarks>
    public GeneticAlgorithm(uint genotypeParamCount, uint populationSize, string pathToInitGenotypeFrom)
    {
        this.PopulationSize = populationSize;
        this.PathToInitGenotypeFrom = pathToInitGenotypeFrom;
    ...
    public void EvaluationFinished()
    {
        ...
        float currentReward = currentPopulation[0].Evaluation;


        //Fire fitness calculation finished event
        if (FitnessCalculationFinished != null && currentReward > bestReward)
        {
    ...
    public void InitializePopulationWithGenotype(IEnumerable<Genotype> population)
    {
        currentPopulation = new List<Genotype>((int)PopulationSize);
        for (int i = 0; i < PopulationSize; i++)
        {
            currentPopulation.Add(Genotype.LoadFromFile(PathToInitGenotypeFrom));
        }
    }
    /// <summary>
    ...
    public static List<Genotype> DefaultSelectionOperator(List<Genotype> currentPopulation)
    {
        ...
        int nbrePopToSelected = GameStateManager.Instance.appSettings.nbSelectedDrone;
        for (int i = 0; i < nbrePopToSelected; i++)
        {
    ...
    public static List<Genotype> DefaultRecombinationOperator(List<Genotype> intermediatePopulation, uint newPopulationSize)
    {
        ...
        while (newPopulation.Count < newPopulationSize)
        {
            ...
            CompleteCrossover(intermediatePopulation[0], intermediatePopulation[1], GameStateManager.Instance.appSettings.defCrossSwapProb, out offspring1, out offspring2);
    ...
    public static void DefaultMutationOperator(List<Genotype> newPopulation)
    {
        foreach (Genotype genotype in newPopulation)
        {
            if (randomizer.NextDouble() < GameStateManager.Instance.appSettings.defMutationPerc)
                MutateGenotype(genotype, GameStateManager.Instance.appSettings.defMutationProb, GameStateManager.Instance.appSettings.defMutationAmount);
        }
    }
    #endregion
